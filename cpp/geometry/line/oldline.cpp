...
ostream& operator<<(ostream &os, TLine l)
{
	os << l.f << " - " << l.t;
	return os;
}
...
bool lineleftpoint(TLine &a, TPoint p)
{
	if (a.f.y==a.t.y) return a.f.x<p.x;
	//return (det(a.f-p,p-a.t)>T(0))^(a.f.y>a.t.y);
...
bool lineleft(const TLine &a, const TLine &b, TPoint p)
{
	T x1=p.x,x2=p.x;
...
bool statsort(const int a, const int b)
{
	if (a>=0 && b>=0)
		return lineleft(line[a],line[b],sweep_point);
	if (a<0 && b<0) return false;
	if (a<0) {
		if (on_lineseg(line[b],sweep_point)) return false;
		return !lineleftpoint(line[b],sweep_point);
	}
	if (on_lineseg(line[a],sweep_point)) return false;
	return lineleftpoint(line[a],sweep_point);
}

/*
bool statsort(const int a, const int b)
{
	cout << (double)sweep_point.x << "," << (double)sweep_point.y << "    ";
	bool c=a_statsort(a,b);
	cout << a << "," << b << " => " << c << endl;
	return c;
}
*/
...
typedef multiset<int,s_statsort> Tstat;
...
bool llb_sort(const int &a, const int &b)
{
...
{
	TEvent e;
	//cout << "Checking if line segments " << i1 << " and " << i2 << " intersects" << endl;
	if (lineseg_intersect_inclusive(line[i1],line[i2],&e.p)) {
	//if (line_intersect(line[i1],line[i2],&e.p)) {
		if (e.p>p) {
#ifdef DBG
...
{			
	...
	while (!eventq.empty()) {
		...
		sort(toinsert.begin(),toinsert.end(),llb_sort);
