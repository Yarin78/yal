...
int main(void)
{
	...
	for(int i=0;i<n;) {
		int x1=rand()%size;
		int y1=rand()%size;
		int x2=x1+rand()%(max*2)-max;
		int y2=y1+rand()%(max*2)-max;
		...
		for(int j=0;(j<i) && ok;j++) {
			if (lineseg_contain(lines[j],lines[i])) ok=0;
			if (lineseg_contain(lines[i],lines[j])) ok=0;
			if (on_lineseg(lines[j],p1) && !(p1==lines[j].f) && !(p1==lines[j].t)) ok=0;
			if (on_lineseg(lines[j],p2) && !(p2==lines[j].f) && !(p2==lines[j].t)) ok=0;
		}
		if (!ok) continue;
