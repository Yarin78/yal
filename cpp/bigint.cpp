// Copyright (c) Derek Kisman (SnapDragon)  dkisman@acm.org
...
public:
	...
	BigInt &operator+=(const BigInt &b) {
		...
		if( car == -1 ) {
			neg = !neg;
			vector<CT>::iterator it = ch.begin();
	...
	BigInt &operator*=(const BigInt &b) {
		...
		vector<CT>::const_iterator it;
		vector<CT> zeros;
		for( it = b.ch.begin(); it != b.ch.end(); it++ ) {
			BigInt b3 = b2 * *it;
	...
	int operator%(int x) const {
		int ret = 0; x = abs(x);
		vector<CT>::const_reverse_iterator it = ch.rbegin();
	...
	inline bool operator>=(const BigInt &b) const {return !(*this < b);}
	inline bool operator!=(const BigInt &b) const {return !(*this == b);}
	inline BigInt &operator>?(BigInt &b) {return (*this>=b) ? *this : b;}
	inline BigInt &operator<?(BigInt &b) {return (*this<=b) ? *this : b;}
	inline const BigInt &operator>?(const BigInt &b) const
	...
	i64 toint() const {
		vector<CT>::const_reverse_iterator it;
		i64 ret = 0;
		for( it = ch.rbegin(); it != ch.rend(); it++ )
	...
	string tostring() const {
		...
		vector<CT>::const_iterator it = ch.begin();
		string ret;
		int i, x;
		while( it != ch.end() ) {
			x = *it;
